...
В теории если в блок 	-- Unlock Gemini воткнуть любые домены то они потом пойдут через варп.
...
**Готово! 🎉**
Теперь при попытке зайти на Gemini, роутер спросит IP-адрес у `sing-box`. `sing-box` выдаст фейковый IP `10.255.x.x`. Трафик полетит в виртуальный туннель и завернется в Cloudflare WARP. 
