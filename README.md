...
3. [Установка в 1 команду (Рекомендуется)](#-установка-в-1-команду-рекомендуется)
4. [Удаление утилиты](#-удаление-утилиты)
5. [Частые вопросы (FAQ)](#-частые-вопросы-faq)
6. [Поддержать проект](#-поддержать-проект)
7. [Ручная Установка (Для продвинутых пользователей)](#-ручная-установка-для-продвинутых-пользователей)
...
Скрипт автоматически определяет архитектуру процессора и проверяет совместимость ОС перед установкой.
...
```
*Во время установки скрипт проверит совместимость вашей системы, установит необходимые зависимости и задаст несколько вопросов (добавление популярных нейросетей в списки и выбор фейковой подсети).*
...
<details>
<summary><b>Какие архитектуры поддерживаются?</b></summary>
<br>
Скрипт автоматически определяет архитектуру вашего процессора и скачивает подходящие бинарные файлы. Поддерживаются серверы на <code>x86_64</code> (amd64), <code>aarch64</code> (arm64, например Oracle Cloud ARM) и <code>armv7l</code>.
</details>
...
Чтобы `sing-box` мог подключиться к WARP, нам нужно сгенерировать личный профиль через утилиту `wgcf`. Определите архитектуру вашего сервера и выполните команды по очереди:
