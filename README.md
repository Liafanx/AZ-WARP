...
|---|---|
| `ANTIZAPRET_WARP=n` + `VPN_WARP=n` | WARPER работает для клиентов AntiZapret (пока что) |
...
FullVPN-клиенты → все → встроенный WARP автора
```

**Ключевой момент:** WARPER патчит только `kresd@1` (который обслуживает AntiZapret-клиентов (пока что)), не затрагивая `kresd@2`. Это позволяет двум системам WARP работать параллельно без конфликтов.

### Автоматическое использование существующих ключей

При установке WARPER автоматически ищет WARP-ключи в следующем порядке:

1. `/etc/wireguard/warp.conf` — ключи от встроенного WARP AntiZapret
...
- возвращать fake-ip для этого домена (только для AntiZapret-клиентов (пока что));
