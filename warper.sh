...
# Fake-подсеть по умолчанию. 100.64.0.0/10 — CGNAT-диапазон (RFC 6598),
# в интернете не маршрутизируется. 198.18/198.20 использовать нельзя:
# 198.18.0.0/15 занят AntiZapret, а 198.20.0.0/16 — реальное публичное
# пространство, клиенты потеряли бы доступ к живым хостам в нём.
DEFAULT_SUBNET="100.85.0.0/16"
