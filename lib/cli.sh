...
cli_ip_ranges_content() {
    ...
    # Шапка имеет фиксированный маркер последней строки:
    #   "# Или в меню: Управление IP-подсетями → Синхронизировать"
    # Всё что после этой строки + одной пустой строки = пользовательский контент.
    # Это позволяет сохранять пользовательские комментарии в начале списка.

    awk '
    BEGIN { found_end = 0; skip_next_blank = 0 }
    {
        if (!found_end) {
            if ($0 ~ /Управление IP-подсетями/) {
                found_end = 1
                skip_next_blank = 1
            }
            next
        }
        # Пропускаем одну пустую строку сразу после маркера шапки
        if (skip_next_blank) {
            skip_next_blank = 0
            if ($0 ~ /^[[:space:]]*$/) {
                next
